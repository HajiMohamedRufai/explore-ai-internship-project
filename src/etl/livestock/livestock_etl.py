...
def database_upload(df, df1, df2):
    ...
    cattle_df.write.jdbc(url=jdbcUrl, table="livestock_cattle_prices", mode="append", properties=properties)
    lamb_df.write.jdbc(url=jdbcUrl, table="livestock_sheep_prices", mode="append", properties=properties)
    pork_df.write.jdbc(url=jdbcUrl, table="livestock_pork_prices", mode="append", properties=properties)
    spark.stop()
